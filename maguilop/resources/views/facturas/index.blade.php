            ...
            <tr class="hover:bg-gray-50 transition">
                ...
                @if ($loop->first)
                    <td class="text-center align-middle" rowspan="{{ $factura->detalles->count() }}">
                        <div style="height: 100%; display: flex; justify-content: center; align-items: center; gap: 8px;">
                            @if($permisos::tienePermiso('Factura', 'editar'))
                                <a href="{{ route('facturas.edit', $factura->FacturaID) }}"
                                   class="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded-circle"
                                   title="Editar">
                            ...
                            @if($permisos::tienePermiso('Factura', 'eliminar'))
                                <form action="{{ route('facturas.destroy', $factura->FacturaID) }}" method="POST"
                                      onsubmit="return confirm('¿Estás seguro de eliminar esta factura?')" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                            class="bg-red-600 hover:bg-red-700 text-white p-2 rounded-circle"
                                            title="Eliminar">
                            ...
                            @if($permisos::tienePermiso('Factura', 'exportar'))
                                <a href="{{ route('facturas.pdf', $factura->FacturaID) }}"
                                   class="bg-gray-700 hover:bg-gray-900 text-white p-2 rounded-circle"
                                   title="Generar PDF">
